...
class Button:
    def __init__(self,screen,XShift,YShift,buttonSize,buttonColor,buttonText,SCREEN_WIDTH,SCREEN_HEIGHT):
        ...
        self.buttonRect = self.buttonSurface.get_rect(center=(SCREEN_WIDTH/2 + self.buttonYShift,(SCREEN_HEIGHT/2)+self.buttonYShift))
        self.buttonTextRect = self.buttonTextSurface.get_rect(center=(SCREEN_WIDTH/2 + self.buttonYShift,(SCREEN_HEIGHT/2)+self.buttonYShift))

    def DecoButton(self, cardFilePath):
        if pygame.mouse.get_pos()[0] in range(self.buttonRect.left,self.buttonRect.right) and pygame.mouse.get_pos()[1] in range(self.buttonRect.top,self.buttonRect.bottom): #check to see if mouse is hovering the button
            ...
            self.buttonTextRect = self.buttonText.get_rect(center=(self.SCREEN_WIDTH/2 + self.buttonXShift,(self.SCREEN_HEIGHT/2)+self.buttonYShift)) # move the text into position
            ...
            self.screen.blit(self.buttonDecoSurf,self.buttonDecoRect0)
            self.screen.blit(self.buttonDecoSurf,self.buttonDecoRect1)
            ...
            if self.checkMouseUp.type == pygame.MOUSEBUTTONUP:
                ...
                print("button clicked") # delete this once you get the actual code for the game
